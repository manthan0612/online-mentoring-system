...
FROM
    ...
    INNER JOIN `oms`.`branch_master` 
        ...
        if(isset($_GET['fid']))
{
   $d= mysqli_query($con, "update faculty_master set staus=1 where f_id='{$_GET['fid']}'") or die(mysqli_error($con));
   if($d)
   {
       echo "<script>alert('Active Successfully');window.location='dis-faculty.php';</script>";
   }
}
 ...
 $i=1;
                        while($row= mysqli_fetch_array($student))
                        {
                            ...
                            echo "<td><a href='display-deleted-faculty-data.php?fid={$row['f_id']}' onclick='return confirm_active()'>Active</a></td>";
                            echo "</tr>";
                            $i++; 
                            }
